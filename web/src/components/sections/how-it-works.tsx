  ...
  return (
    <section id="how-it-works" className="container-page py-20 md:py-28">
      <div className="max-w-2xl">
        <p className="text-xs font-semibold uppercase tracking-widest text-ink-soft">Kako funkcioniše</p>
        <h2 className="mt-3 font-display text-balance text-3xl font-bold tracking-[-0.02em] text-foreground sm:text-4xl md:text-5xl">
          Od slike sa telefona do mirne table.
        </h2>
        <p className="mt-4 text-base text-ink-soft sm:text-lg">
          Četiri koraka. Bez nepotrebnog dopisivanja i bez iznenađenja kad zazvonimo na vrata.
        </p>
      </div>

      <ol className="mt-12 grid gap-3 md:hidden">
        {howItWorks.map((s, i) => (
          <li key={s.step} className="relative flex gap-4 rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col items-center">
              <span className="grid h-9 w-9 place-items-center rounded-full bg-foreground text-sm font-semibold text-background">{s.step}</span>
              {i < howItWorks.length - 1 && <span className="my-2 w-px flex-1 bg-border" />}
            </div>
            <div className="pb-3">
