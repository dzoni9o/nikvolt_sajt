        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_1fr] lg:gap-16">
          <div className="max-w-2xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-background/60 px-3 py-1.5 text-xs font-medium text-ink-soft backdrop-blur">
              ...
              Dostupni odmah u {site.city}u · odgovor {site.responseTime}
            </div>
            ...
            <h1 className="font-display text-balance text-[2.6rem] leading-[1.02] font-bold tracking-[-0.02em] text-foreground sm:text-6xl lg:text-[4.25rem]">
              Električar u {site.city}u{" "}
              <span className="relative inline-block">
                <span className="relative z-10">bez nagađanja</span>
                <span
                  aria-hidden
            ...
            <p className="mt-6 max-w-xl text-pretty text-lg text-ink-soft sm:text-xl">
              Pošalji slike problema. Dobij procenu kvara, okvirnu cenu i
              vreme izlaska na teren — najčešće u roku od sat vremena.
            </p>
            ...
            <ul className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-ink-soft">
              <li className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-foreground" /> Licencirano · osigurano</li>
              <li className="flex items-center gap-2"><Clock4 className="h-4 w-4 text-foreground" /> Izlazak istog dana</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-foreground" /> {site.jobsCompleted.toLocaleString("sr-RS")}+ završenih intervencija</li>
            </ul>
          </div>
    ...
    <div className="relative mx-auto w-full max-w-md lg:max-w-none">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border bg-card shadow-xl">
        <div className="flex items-center gap-2 border-b border-border bg-paper/60 px-4 py-3">
          ...
          <span className="ml-3 text-xs font-medium text-ink-soft">Pošalji problem</span>
        </div>
        <div className="space-y-4 p-5">
          <div className="grid grid-cols-3 gap-2">
            <PhotoTile label="Tabla" tone="warn" />
            <PhotoTile label="Utičnica" tone="ok" />
            <PhotoTile label="Pregoreo kabl" tone="bad" />
          </div>

          <div className="rounded-2xl border border-border bg-paper/70 p-4">
            <div className="flex items-center gap-2 text-xs font-medium text-ink-soft">
              <Camera className="h-3.5 w-3.5" /> Simptomi
            </div>
            <div className="mt-2 flex flex-wrap gap-1.5">
              {["Varničenje", "Miris paljevine", "Topla tabla"].map((c) => (
          ...
          <div className="rounded-2xl border border-foreground/10 bg-foreground p-4 text-background">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-widest text-background/60">Procena</div>
                <div className="mt-1.5 text-base font-semibold leading-tight">Visok rizik · verovatno luk na kuhinjskom kolu</div>
              </div>
              <span className="shrink-0 rounded-full bg-emergency px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-emergency-foreground">Visok</span>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-2">
              <div className="rounded-lg bg-background/10 p-2.5">
                <div className="text-[11px] uppercase tracking-wider text-background/60">Okvirno</div>
                <div className="text-sm font-semibold">8.500 – 14.000 RSD</div>
              </div>
              <div className="rounded-lg bg-background/10 p-2.5">
                <div className="text-[11px] uppercase tracking-wider text-background/60">Najraniji termin</div>
                <div className="text-sm font-semibold">Danas, 18:30</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="absolute -bottom-5 -left-5 hidden rotate-[-4deg] rounded-2xl border border-border bg-card p-3 shadow-lg sm:block">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-semibold text-foreground">Izolacija proverena</div>
            <div className="text-[11px] text-ink-soft">Kalibrisani megaomometar · zapisnik</div>
          </div>
        </div>
      </div>
    </div>
